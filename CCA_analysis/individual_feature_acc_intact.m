...
Participant_dir = 'Dataset/Intact_dataset';
...
disp('-----------------------------------------')
disp('      Construct Training Dataset         ')
disp('-----------------------------------------')

tr_data = getdataset(Participant_dir, 'intact', 'train', 1000, 1:10, class_list);

disp('-----------------------------------------')
disp('       Construct Testing Dataset         ')
disp('-----------------------------------------')

te_data = getdataset(Participant_dir, 'intact', 'test', 1000, 1:10, class_list);
...
for f = 1:length(features_list)
    ...
    if ~exist(['Features/extracted_intact/tr_' features_list{f} '.mat'],'file')
        disp(['F(' num2str(f) '/' num2str(length(features_list)) '): ' features_list{f}])
        tr_features = getfeature(tr_data,features_fn{f},winsize,wininc);
        mysave(['Features/extracted_intact/tr_' features_list{f} '.mat'],tr_features)
    else
        tr_features = load(['Features/extracted_intact/tr_' features_list{f} '.mat'],'feature');
        tr_features = tr_features.feature;
    end
    
    if ~exist(['Features/extracted_intact/te_' features_list{f} '.mat'],'file')
        te_features = getfeature(te_data,features_fn{f},winsize,wininc);
        mysave(['Features/extracted_intact/te_' features_list{f} '.mat'],te_features)
    else
        te_features = load(['Features/extracted_intact/te_' features_list{f} '.mat'],'feature');
...
%         continue
%     end
%     if ~exist(['Features/results_intact/' features_list{f} '_ws_acc.mat'],'file')
%         tr_features = load(['Features/extracted_intact/tr_' features_list{f} '.mat'],'feature');
%         tr_features = tr_features.feature;
% 
%         te_features = load(['Features/extracted_intact/te_' features_list{f} '.mat'],'feature');
...
%         save(['Features/results_intact/' features_list{f} '_ws_acc.mat'],'ws_acc');
%         save(['Features/results_intact/' features_list{f} '_ws_cmat.mat'],'ws_cmat');
%     end
% end
...
%         continue
%     end
%     if ~exist(['Features/results_intact/' features_list{f} '_bs_acc.mat'],'file')
%         tr_features = load(['Features/extracted_intact/tr_' features_list{f} '.mat'],'feature');
%         tr_features = tr_features.feature;
% 
%         te_features = load(['Features/extracted_intact/te_' features_list{f} '.mat'],'feature');
...
%         save(['Features/results_intact/' features_list{f} '_bs_acc.mat'],'bs_acc');
%         save(['Features/results_intact/' features_list{f} '_bs_cmat.mat'],'bs_cmat');
%     end
% end
...
%     if ~exist(['Features/results_intact/' featuresets{f} '_ws_acc.mat'],'file')
%         
%         for fi = 1:length(featuresets_components{f})
%            if fi == 1
%                tr_features = load(['Features/extracted_intact/tr_' featuresets_components{f}{fi} '.mat'],'feature');
%                tr_features = tr_features.feature;
%                te_features = load(['Features/extracted_intact/te_' featuresets_components{f}{fi} '.mat'],'feature');
%                te_features = te_features.feature;
%            else
...
%                
%                tmp = load(['Features/extracted_intact/te_' featuresets_components{f}{fi} '.mat'],'feature');
%                tmp = tmp.feature;
...
%         save(['Features/results_intact/' featuresets{f} '_ws_acc.mat'],'ws_acc');
%         save(['Features/results_intact/' featuresets{f} '_ws_cmat.mat'],'ws_cmat');
%     end
%     
% end
...
disp('-----------------------------------------')
disp('        Single Rep - Featuresets         ')
disp('-----------------------------------------')
remove_class = [6,7,9];
for f = 1:length(featuresets)
  %if ~exist(['Features/results_intact/' featuresets{f} '_sr_acc.mat'],'file')
        ...
        te_reps = unique(te_features(:,3));
        tr_reps = te_reps(1:2);
        te_reps = te_reps(3:end);
        for p = 1:length(participants)

            %tr_p = tr_features(tr_features(:,1) == participants(p),:);
            %tr_p = 
            tr_p = te_features(te_features(:,1) == participants(p),:);
            tr_p = tr_p(ismember(tr_p(:,3),tr_reps),:);
        ...
        %save(['Features/results_intact/' featuresets{f} '_sr_acc.mat'],'sr_acc');
        %save(['Features/results_intact/' featuresets{f} '_sr_cmat.mat'],'sr_cmat');
    %end
...
% remove_class = [6,7,9];
% for f = 1:length(featuresets)
%     if ~exist(['Features/results_intact/' featuresets{f} '_bs_acc.mat'],'file')
%         
%         for fi = 1:length(featuresets_components{f})
%            if fi == 1
%                tr_features = load(['Features/extracted_intact/tr_' featuresets_components{f}{fi} '.mat'],'feature');
...
%         save(['Features/results_intact/' featuresets{f} '_bs_acc.mat'],'bs_acc');
%         save(['Features/results_intact/' featuresets{f} '_bs_cmat.mat'],'bs_cmat');
%     end
% end
...
remove_class = [6,7,9];
for f = 1:length(featuresets)
    %if ~exist(['Features/results_intact/' featuresets{f} '_lscca_acc.mat'],'file')
        disp(['FS: ' featuresets{f}])
        for fi = 1:length(featuresets_components{f})
           if fi == 1
               tr_features = load(['Features/extracted_intact/tr_' featuresets_components{f}{fi} '.mat'],'feature');
               tr_features = tr_features.feature;
        ...
        for p1 = 1:length(participants)
            ...
            for p2 = 1:length(participants)
                ...
                LDA_mdl = fitcdiscr(train_data(:,4:end), train_data(:,2),'discrimtype','pseudolinear');
        ...
        b = squeeze(mean(bs_acc,3));
        b = sum(b,2)/(size(b,2)-1);
        
        disp(['F(' num2str(f) '/' num2str(length(featuresets)) ') ' featuresets{f} ': ' num2str(mean(b)) ' ' num2str(std(b))])
        %save(['Features/results_intact/' featuresets{f} '_lscca_acc.mat'],'bs_acc');
        %save(['Features/results_intact/' featuresets{f} '_lscca_cmat.mat'],'bs_cmat');
    %end
end