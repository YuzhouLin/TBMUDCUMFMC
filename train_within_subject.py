...
from PrepareAndLoadData.load_prepared_dataset_in_dataloaders import load_dataloader_for_training_without_TL,\
    load_dataloaders
import PrepareAndLoadData.load_raw_data_and_prepare_dataset_within as lrdapdw
...
if __name__ == "__main__":
    ...
    for p in range(0,5):
        ...
        lrdapdw.read_data(path="Dataset/" + population + "_dataset",participant_num=p+1, window_size=151)


        pretrain_raw_convNet(path="Dataset/processed_dataset", filter_size=(1,26))

        predictions, ground_truth = test_network_raw_convNet(filter_size=(1,26))
