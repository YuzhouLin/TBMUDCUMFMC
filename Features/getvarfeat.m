%
% GETVARFEAT Gets the Variance feature.
% Variance = 2nd Temporal Moment
%
% feat = getvarfeat(x,winsize,wininc,[])
%
% This function computes the variance feature of the signals in x,
...
%
% Outputs
%    feat:     variance value in a 2 dimensional matrix
%              dim1 window
%              dim2 feature (col i is the features for the signal in column i of x)

function feat = getvarfeat(x,winsize,wininc,option) %#ok<INUSD>
...
for i = 1:numwin
    ...
    % feat(i,:) = var(curwin);
    feat(i,:) = mean(curwin.^2);
    % feat(i,:) = sum(curwin.^2)./(winsize-1);
