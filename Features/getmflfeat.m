%
% getMFLfeat: gets the Maximum Fractal Length feature.
% Only when k = 1
% 
% feat = getmflfeat(x,winsize,wininc,[])
%
% This function computes the maximum fractal length feature 
...
%
% Outputs
%    feat:     maximum fractal length value in a 2 dimensional matrix
%              dim1 window
%              dim2 feature (col i is the features for the signal in column i of x)

function feat = getmflfeat(x,winsize,wininc,option) %#ok<INUSD>
...
for i = 1:numwin
   ...
   feat(i,:) = log10(sum(abs(diff(curwin))));
