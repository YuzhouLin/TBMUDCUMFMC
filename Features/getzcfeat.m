%
% GETZCFEAT Gets the Zero Crossing feature.
%
% feat = getzcfeat(x,winsize,wininc,threshold)
%
% This function computes the zero crossing feature of the signals in x,
...
%
% Outputs
%    feat:     zero crossing value in a 2 dimensional matrix
%              dim1 window
%              dim2 feature (col i is the features for the signal in column i of x)

function feat = getzcfeat(x,winsize,wininc,option)
...
for i = 1:numwin
   ...
   clear curwin
   con_a = (abs(curwin1-curwin2)>=threshold);
   con_b = ((curwin1.*curwin2)<0);
   con_ab = con_a + con_b;
   feat(i,:) = (sum(con_ab==2));
   clear curwin1 curwin2 con_a con_b con_ab
