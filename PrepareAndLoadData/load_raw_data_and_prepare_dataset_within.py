...
def get_data_and_process_it_from_file(get_train_data, participant_num, path, number_of_gestures=7, number_of_cycles=16, window_size=151,
                                      size_non_overlap=50):
    ...
    for participant_directory in participants_directories:
        ...
        if participant_directory[-1] != str(participant_num):
            continue
        print("Preparing data of: " + participant_directory)
        examples_participants, labels_participant = [], []

        for cycle in range(number_of_cycles):
            path_emg = path + "/" + participant_directory + "/" + "%s/EMG/Amp_EMG_gesture_%d_" % (train_or_test_str,
                                                                                                  cycle)
            examples, labels = [], []
...
def read_data(path, participant_num, number_of_gestures=7, number_of_cycles=16, window_size=200, size_non_overlap=50):
    print("Loading and preparing datasets...")
    'Get and process the train data'
    print("Taking care of the training data...")
    list_dataset_train_emg, list_labels_train_emg = get_data_and_process_it_from_file(get_train_data=True, participant_num = participant_num,
                                                                                      path=path,
                                                                                      number_of_gestures=
                                                                                      number_of_gestures,
                                                                                      ...
                                                                                      size_non_overlap=size_non_overlap)
    np.save("Dataset/processed_dataset/RAW_INT_train", (list_dataset_train_emg, list_labels_train_emg))
    print("Finished with the training data...")
    'Get and process the test data'
    print("Starting with the test data...")
    list_dataset_train_emg, list_labels_train_emg = get_data_and_process_it_from_file(get_train_data=False,participant_num=participant_num,
                                                                                      path=path,
                                                                                      number_of_gestures=
                                                                                      number_of_gestures,
                                                                                      number_of_cycles=number_of_cycles,
                                                                                      window_size=window_size,
                                                                                      size_non_overlap=size_non_overlap)
    np.save("Dataset/processed_dataset/RAW_INT_test", (list_dataset_train_emg, list_labels_train_emg))
    print("Finished with the test data")
    

if __name__ == '__main__':
    read_data(path="Dataset/Intact_dataset", window_size=151)
